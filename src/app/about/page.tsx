  ...
  return <main className="overflow-hidden bg-background">
    <section className="container-wide pt-5 pb-16 md:pt-8 md:pb-24">
      <div className="relative min-h-[440px] overflow-hidden rounded-[1.75rem] bg-navy md:min-h-[540px]">
        ...
        <div className="relative flex min-h-[440px] max-w-3xl flex-col justify-end gap-5 p-7 md:min-h-[540px] md:p-12"><p className="font-display text-sm uppercase tracking-[.15em] text-gold">The Charloxy story</p><h1 className="display-tight text-5xl text-background md:text-7xl">Safe and Secure Delivery.<br className="hidden md:block"/> <span className="text-gold">Every Time.</span></h1><p className="max-w-xl text-base leading-7 text-background/80">Charloxy Transport is a growing logistics company based in Johannesburg, dedicated to providing reliable, personalized transport solutions.</p></div>
      </div>
    </section>

    <section className="container-wide pb-20 md:pb-28"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-20"><div><p className="font-display text-sm uppercase tracking-[.15em] text-gold">Who we are</p><h2 className="display-tight mt-3 text-5xl text-navy md:text-7xl">The team behind the <span className="text-gold">move.</span></h2></div><div className="grid gap-6 text-base leading-7 text-ink-muted md:grid-cols-2"><p>We have built our reputation by providing professional residential and commercial moving services. We guarantee that we will deliver all of your goods and furniture in the exact same condition that we collected them.</p><p>Our team has the requisite experience to meet every customer’s needs, beginning with keeping each and every item that we transport safe and secure.</p><p>Our reasonable rates, careful handling, and track record of satisfied customers have made us the ideal choice for many Johannesburg households and businesses needing help with any size and type of move.</p><p>As a division of Charloxy Investment (Pty) Ltd, we have the backing and the vision to become your preferred transport partner, giving you total peace of mind.</p></div></div></section>
