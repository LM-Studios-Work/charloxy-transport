...
export default function Home() {
  return <div className="overflow-hidden bg-background">
    ...
    <section className="bg-paper py-20 md:py-28"><div className="container-wide grid items-center gap-12 lg:grid-cols-2"><div><p className="font-display text-sm uppercase tracking-[.14em] text-gold">Why choose us</p><h2 className="display-tight mt-3 text-5xl text-navy md:text-7xl">Premium service.<br /><span className="text-gold">Peace of mind.</span></h2><p className="mt-6 max-w-lg text-base leading-7 text-ink-muted">Over the years, Charloxy Transport has become the go-to moving company when looking for affordable, reliable, and personalized transport solutions. We offer the best transport services at reasonable prices without ever compromising on quality.</p><ul className="mt-8 grid gap-4 text-sm font-semibold text-navy sm:grid-cols-2">{['Free Estimates', 'Goods in Transit Insurance', 'Safe & Secure Delivery', 'Guaranteed Moving Dates'].map(item => <li key={item} className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold"><Check size={15} /></span>{item}</li>)}</ul></div><div className="photo-card relative h-[360px] bg-navy md:h-[480px]"><img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=1100&q=85" alt="Professional movers carrying furniture" className="size-full object-cover" /><div className="absolute bottom-5 left-5 right-5 rounded-xl bg-gold p-5"><p className="font-display text-3xl uppercase text-navy">Ready to move?</p><Link href="/quote" className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-navy underline underline-offset-4">Get a fast quote <ArrowRight size={16} /></Link></div></div></div></section>

    <section className="container-wide py-20 md:py-28"><div className="rounded-[1.75rem] bg-[#EAF3F7] p-6 md:p-12"><div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between"><div><p className="display-tight text-5xl text-navy md:text-6xl">What our <span className="text-gold">customers</span> say</p></div><div className="flex gap-2"><button type="button" aria-label="Previous reviews" className="grid size-11 place-items-center rounded-full border border-navy/30 text-navy"><ChevronLeft /></button><button type="button" aria-label="Next reviews" className="grid size-11 place-items-center rounded-full bg-gold text-navy"><ChevronRight /></button></div></div><div className="mt-8 grid gap-4 lg:grid-cols-3">{reviews.map(([name, text]) => <article key={name} className="rounded-2xl bg-background p-6"><div className="flex items-center justify-between"><div className="grid size-12 place-items-center rounded-full bg-gold font-display text-xl text-navy">{name[0]}</div><span className="rounded-full bg-gold px-3 py-1 text-xs font-bold text-navy">5★</span></div><p className="mt-8 text-base leading-7 text-navy">“{text}”</p><p className="mt-8 text-sm font-bold text-navy">{name}</p><p className="mt-1 text-xs text-ink-muted">Verified customer</p></article>)}</div></div></section>
